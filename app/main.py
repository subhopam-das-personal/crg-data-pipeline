...
with tab_upload:
    ...
    with col_demo:
        st.write("**Demo files:**")
        if st.button("✅ Clean bundle (18 obs)"):
            ...
            st.session_state.results = None
        if st.button("⚠️ Anomaly bundle (20 obs, 2 broken)"):
...
5. Click **Lineage** tab to see the full DAG: `fhir_bundle_upload → bronze → silver → gold`
        ...
        with col_link:
            ...
            st.link_button("View silver→gold DAG →", _job_url)
